...
func RegisterApis(apis ...system.SysApi) {
	...
	global.Gxva_DB.Find(&[]system.SysApi{}, "path in (?)", apiPaths).Count(&count)
	if count > 0 {
		fmt.Println("插件已安装或存在同名路由")
		return
	}
	err := global.Gxva_DB.Create(&apis).Error
	if err != nil {
		fmt.Println(err)
	}
}
...
func RegisterMenus(menus ...system.SysBaseMenu) {
	...
	global.Gxva_DB.Find(&[]system.SysBaseMenu{}, "name in (?)", menuNames).Count(&count)
	if count > 0 {
		fmt.Println("插件已安装或存在同名菜单")
		return
	}
	parentMenu.ParentId = 0
	err := global.Gxva_DB.Create(&parentMenu).Error
	if err != nil {
		fmt.Println(err)
	}
	...
	err = global.Gxva_DB.Create(&otherMenus).Error
	if err != nil {
		fmt.Println(err)
	}
}
