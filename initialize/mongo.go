...
func (m *mongo) Initialization() error {
	...
	if global.Gxva_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              global.Gxva_CONFIG.Mongo.Uri(),
		Coll:             global.Gxva_CONFIG.Mongo.Coll,
		Database:         global.Gxva_CONFIG.Mongo.Database,
		MinPoolSize:      &global.Gxva_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.Gxva_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.Gxva_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.Gxva_CONFIG.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username:   global.Gxva_CONFIG.Mongo.Username,
			Password:   global.Gxva_CONFIG.Mongo.Password,
			AuthSource: global.Gxva_CONFIG.Mongo.AuthSource,
		},
	}, opts...)
	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	global.Gxva_MONGO = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.Gxva_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.Gxva_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
