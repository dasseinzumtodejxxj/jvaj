...
func (apiService *ApiService) CreateApi(api system.SysApi) (err error) {
	if !errors.Is(global.Gxva_DB.Where("path = ? AND method = ?", api.Path, api.Method).First(&system.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.Gxva_DB.Create(&api).Error
}
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	...
	err = global.Gxva_DB.First(&entity, "id = ?", api.ID).Error // 根据id查询api记录
	if errors.Is(err, gorm.ErrRecordNotFound) {                 // api记录不存在
		return err
	}
	err = global.Gxva_DB.Delete(&entity).Error
	if err != nil {
		return err
	}
...
func (apiService *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	...
	db := global.Gxva_DB.Model(&system.SysApi{})
...
func (apiService *ApiService) GetAllApis() (apis []system.SysApi, err error) {
	err = global.Gxva_DB.Find(&apis).Error
	return
}
...
func (apiService *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	err = global.Gxva_DB.First(&api, "id = ?", id).Error
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	err = global.Gxva_DB.First(&oldA, "id = ?", api.ID).Error
	if oldA.Path != api.Path || oldA.Method != api.Method {
		var duplicateApi system.SysApi
		if ferr := global.Gxva_DB.First(&duplicateApi, "path = ? AND method = ?", api.Path, api.Method).Error; ferr != nil {
	...
	return global.Gxva_DB.Save(&api).Error
}
