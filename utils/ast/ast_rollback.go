...
func RollGormBack(pk, model string) {
	...
	// 如果多个 仅仅删除对应块即可
	// 如果单个 那么还需要剔除import
	path := filepath.Join(global.Gxva_CONFIG.AutoCode.Root, global.Gxva_CONFIG.AutoCode.Server, "initialize", "gorm.go")
...
func RollRouterBack(pk, model string) {
	...
	path := filepath.Join(global.Gxva_CONFIG.AutoCode.Root, global.Gxva_CONFIG.AutoCode.Server, "initialize", "router.go")
	src, err := os.ReadFile(path)
	if err != nil {
		fmt.Println(err)
	}
