...
func NewHuaWeiObsClient() (client *obs.ObsClient, err error) {
	return obs.New(global.Gxva_CONFIG.HuaWeiObs.AccessKey, global.Gxva_CONFIG.HuaWeiObs.SecretKey, global.Gxva_CONFIG.HuaWeiObs.Endpoint)
}
...
func (o *Obs) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	input := &obs.PutObjectInput{
		PutObjectBasicInput: obs.PutObjectBasicInput{
			ObjectOperationInput: obs.ObjectOperationInput{
				Bucket: global.Gxva_CONFIG.HuaWeiObs.Bucket,
				Key:    filename,
			},
	...
	filepath := global.Gxva_CONFIG.HuaWeiObs.Path + "/" + filename
